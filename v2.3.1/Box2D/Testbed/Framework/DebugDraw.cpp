...
//
static void sCheckGLError(const char *str=0)
{
    return;
    if (str)
        cerr << "sCheckGLError:" << str << endl;
...
static GLuint sCreateShaderFromString(const char* source, GLenum type)
{
    ...
    sCheckGLError("after glCreateShader()");
    ...
    sCheckGLError("after glGetShaderiv");
...
static GLuint sCreateShaderProgram(const char* vs, const char* fs)
{
    ...
    sCheckGLError("after sCreateShader(VERTEX)");
    GLuint fsId = sCreateShaderFromString(fs, GL_FRAGMENT_SHADER);
    sCheckGLError("after sCreateShader(FRAGMENT)");
    ...
    sCheckGLError("after glLink");

    glDeleteShader(vsId);
    glDeleteShader(fsId);
    
    sCheckGLError("after glDeleteShaders");

    GLint status = GL_FALSE;
    glGetProgramiv(programId, GL_LINK_STATUS, &status);
    assert(status != GL_FALSE);

    sCheckGLError("after glGetProgramiv");
...
struct GLRenderPoints
{
    ...
    sCheckGLError("after sCreateShaderProg");
    
    m_projectionUniform = glGetUniformLocation(m_programId, "projectionMatrix");
    sCheckGLError("after glGetUniformLocation");
    ...
    sCheckGLError("after glGenBuffers");
    ...
    sCheckGLError("after enableVtxAttArr");
    ...
    sCheckGLError("after vtx");
    
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
    glVertexAttribPointer(m_colorAttribute, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
    glBufferData(GL_ARRAY_BUFFER, sizeof(m_colors), m_colors, GL_DYNAMIC_DRAW);
    
    sCheckGLError("after color");
    
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[2]);
    glVertexAttribPointer(m_sizeAttribute, 1, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
    glBufferData(GL_ARRAY_BUFFER, sizeof(m_sizes), m_sizes, GL_DYNAMIC_DRAW);
    
    sCheckGLError("after size");
